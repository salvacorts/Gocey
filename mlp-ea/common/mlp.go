...
const (
	...
	ScalingFactor = 0.0000000000001 // TODO: Check if this is really useful

	// EvaluationEpochs states how many epochs to train the MLP in the Evaluate operator
	EvaluationEpochs = 10
...
// TODO: Put the interface in go-perceptron go
type MLP struct {
...
func (mlp *MLP) Mutate(rng *rand.Rand) {
	indexNL := utils.RandIntInRange(1, len(mlp.NeuralNet.NeuralLayers)-1, rng)

	// Modifies the weights of certain neurons, at random, depending on the application rate
	for i := range mlp.NeuralNet.NeuralLayers[indexNL].NeuronUnits {
		if rng.Float64() < MutateRate {
			u := utils.RandFloatInRange(-0.1, 0.1, rng) * ScalingFactor

			for j := range mlp.NeuralNet.NeuralLayers[indexNL].NeuronUnits[i].Weights {
				mlp.NeuralNet.NeuralLayers[indexNL].NeuronUnits[i].Weights[j] += u
			}
		}
	}
...
func (mlp *MLP) Crossover(Y eaopt.Genome, rng *rand.Rand) {
	n := len(mlp.NeuralNet.NeuralLayers)

	if len(Y.(*MLP).NeuralNet.NeuralLayers) > n {
		n = len(Y.(*MLP).NeuralNet.NeuralLayers)
	}

	// Perform a multipoint cross-over on each hidden layer
	for i := 1; i < n-1; i++ {
		p1 := neurons(mlp.NeuralNet.NeuralLayers[i].NeuronUnits)
		p2 := neurons(Y.(*MLP).NeuralNet.NeuralLayers[i].NeuronUnits)

		eaopt.CrossGNX(p1, p2, 2, rng)
	}

	// Swap learning rates
	mlp.NeuralNet.L_rate = Y.(*MLP).NeuralNet.L_rate
...
func (mlp *MLP) Clone() eaopt.Genome {
	new := MLP{
		Config: mlp.Config,
		NeuralNet: mn.MultiLayerNetwork{
			L_rate:       mlp.NeuralNet.L_rate,
			T_func:       mlp.NeuralNet.T_func,
			T_func_d:     mlp.NeuralNet.T_func_d,
			NeuralLayers: make([]mn.NeuralLayer, len(mlp.NeuralNet.NeuralLayers)),
		},
	}

	copy(new.NeuralNet.NeuralLayers, mlp.NeuralNet.NeuralLayers)

	for i := range new.NeuralNet.NeuralLayers {
		new.NeuralNet.NeuralLayers[i].Length = mlp.NeuralNet.NeuralLayers[i].Length

		new.NeuralNet.NeuralLayers[i].NeuronUnits =
			neurons(mlp.NeuralNet.NeuralLayers[i].NeuronUnits).Copy().(neurons)
	}
