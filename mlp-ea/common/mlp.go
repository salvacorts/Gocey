...
func (mlp MLP) Evaluate() (float64, error) {
	...
	predictions := utils.PredictN(&mlp.nn, *mlp.config.ValidationSet)
	roundedPredictions := utils.RoundPredictions(predictions)

	_, accuracy := utils.AccuracyN(roundedPredictions, *mlp.config.ValidationSet)
